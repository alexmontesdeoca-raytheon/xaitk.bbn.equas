...
// }
@Component({
    selector: 'jhi-one-shot-v2',
    templateUrl: './one-shot-v2.component.html',
    providers: [MessageService],
    styleUrls: ['blur-game.scss']
})
export class OneShotV2Component implements OnInit, OnDestroy {
    account: Account;
    ...
    oneShotAnswer = new OneShotAnswerV2();
    ...
    bonusPay = 0;
    maxBonusPayout = 0;
    allLikertQuestionsOneshot = require('./likertQuestions-Oneshot.json');
    likertQuestions = [];
    ...
    getNextTrial(isGameMode: boolean) {
        ...
        this.saveFeedback(false).then(result => {
            ...
            this.globalExplanationService
                .getNextOneShotV2GameQuestion(this.getWorkerId(), this.getAssignmentId(), forceQuestionId, this.modality)
                .subscribe(response => {
                    // this.trial += 1;
                    ...
                    if (this.reachedAssignmentQuota() || this.reachedMaxQuota()) {
                        ...
                        this.loadRankings();
                        this.setRandomLikertQuestions(4);
                    }
                });
        });
    }
    loadRankings() {
        ...
        this.globalExplanationService.getRanking(this.exampleImages[this.exampleImageIndex]).subscribe(
            rankings => {
                this.exampleImageRankings = JSON.parse(JSON.stringify(rankings.body));
                //Shuffle the first 20 features
                const shuffleFeatures = this.route.snapshot.queryParams['shuffleFeatures']
                    ? this.route.snapshot.queryParams['shuffleFeatures']
                    : 'false';
                if (shuffleFeatures === 'true') {
                    this.exampleImageRankings.units = this.shuffleArrayFirstN(
                        [].concat(this.exampleImageRankings.units),
                        this.maxUnits - 2
                    );
                }
                ...
                this.oneShotAnswer = new OneShotAnswerV2();
    ...
    didUserChangeSort(): boolean {
        // return true;
        for (let i = 0; i < this.maxUnits - 1; ++i) {
            if (this.exampleImageRankings.initalSort[i].unit !== this.exampleImageRankings.units[i].unit) {
                return true;
            }
        }
        return false;
    ...
    resetWorkerId() {
        this.globalExplanationService.deleteOneShotV2AnswersByWorkerId(this.getWorkerId()).subscribe(response => {
            this.messageService.add({
                severity: 'success',
                summary: 'Success!',
    ...
    isBelowExcluded(unit): boolean {
        const excludeBelowIndex = this.exampleImageRankings.units.indexOf(this.excludeBelow);
        if (this.exampleImageRankings.units.indexOf(unit) > excludeBelowIndex) {
            return true;
        }
        return false;
    }
                ...
                accept: () => {
                    if (this.showProgressSpinner === false) {
                        ...
                        try {
                            ...
                            // const initalSort = this.exampleImageRankings.initialSort.map(unit => {
                            //     return unit.label + '-' + unit.unit;
                            // });
                            // this.oneShotAnswer.initalSort = initalSort.join(',');
                            newRankings['stats'] = this.oneShotAnswer;
                            this.globalExplanationService.sumbitRanking(this.getCategory(), newRankings).subscribe(
                                results => {
                                    this.messages.push({
                                        severity: 'success',
                                    ...
                                    this.scoredResults.scores.pctImprovement =
                                        this.scoredResults.scores.f1_after - this.scoredResults.scores.f1_before;
                                    this.trialScore = this.scoredResults.scores.pctImprovement; //this.normalize(this.scoredResults.scores.f1_after, 0, .5, 0, 100);
                                    this.hideSpinner();
    ...
    drop(event: CdkDragDrop<string[]>) {
        ...
        this.oneShotAnswer.totalMoveCount += 1;
    }
    ...
    saveRanking(submitForScore: boolean) {
        this.oneShotAnswer.excludedCount = 0;
        const newRankings: Rankings = JSON.parse(JSON.stringify(this.exampleImageRankings));
        let excludeBelowIndex = 9999;
        // newRankings.units.splice(excludeBelowIndex, 1);
        for (let newIndex = 0; newIndex < newRankings.units.length; newIndex++) {
            let unit = newRankings.units[newIndex];
            if (unit.unit === this.excludeBelow.unit) {
                excludeBelowIndex = newIndex; // store excludeBelowIndex
                newRankings.units.splice(excludeBelowIndex, 1); // Remove Exclude below this line from array.
                unit = newRankings.units[newIndex];
                if (excludeBelowIndex === 0) {
                    // Hack: Fixes divide by zero error. Include at least one orginal unit if the user moves the excluce line to the top.
                    excludeBelowIndex = 1;
                }
            }

            if (unit.index < this.maxUnits - 1) {
                // Is the old index in the range of the visble working units
                if (newIndex >= excludeBelowIndex) {
                    unit.index = this.exampleImageRankings.initalSort.length + newIndex;
                    unit['excluded'] = true;
                    this.oneShotAnswer.excludedCount += 1;
                } else {
                    unit.index = newIndex;
                }
            }
        ...
        // sort by index
        newRankings.units.sort((a, b) => (a.index < b.index ? -1 : b.index < a.index ? 1 : 0));
        //Swap iou scores and fix indexes
        for (let index = 0; index < this.exampleImageRankings.initalSort.length; index++) {
            let ogUnit = this.exampleImageRankings.initalSort[index];
            let newUnit = newRankings.units[index];
            newUnit.iou = ogUnit.iou;
            newUnit.index = ogUnit.index;
        }

        if (submitForScore) {
            this.submitOneShot(newRankings);
            ...
            //     });
            // } else {
            //     this.globalExplanationService.sumbitRanking(this.getCategory(), newRankings).subscribe(results => {
            //         this.scoredResults = results.body;
            //         this.hideSpinner();
            //     },
            //     (error) => {
            ...
            saveAs(
                new Blob([JSON.stringify(newRankings, null, 2)], { type: 'text' }),
                this.exampleImages[this.exampleImageIndex] + '-sorting.json'
            );
    ...
    newUnitLabel(oldUnit: Unit): string {
        const newUnit = this.newUnitLabels.units[oldUnit.unit];
        if (newUnit) {
            oldUnit['newLabel'] = newUnit.label;
            return newUnit.label;
        } else {
